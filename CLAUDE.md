...
**Two routing paths in the bot:**

1. **Agent path (default):** ALL messages go to `agent.run()` → Anthropic API with tool calling (Sonnet). The agent has 30+ tools for email, calendar, CRM, sales pipeline, web search, and more. It decides which tools to use based on the user's intent. Conversation history is included for follow-up support.
2. **Document path:** Translation, anonymization, file creation keywords → regex-based `detectIntent()` → deterministic bot logic with AI-generated content. Only triggered when a document operation is detected.

When the agent returns no response, fallback goes to `callOpenClaw()` → OpenClaw gateway → Claude Opus for general chat. The chat path now receives unified context (entities, agent summaries, file context) via `context-manager.js`.
...
- **Railway env vars:**
  ...
  - `SF_CLIENT_ID` or `SALESFORCE_CLIENT_ID` — Salesforce Connected App consumer key
  - `SF_CLIENT_SECRET` or `SALESFORCE_CLIENT_SECRET` — Salesforce Connected App consumer secret
  - `SF_LOGIN_URL` or `SALESFORCE_LOGIN_URL` — Salesforce login URL (use org domain e.g. `https://al-enterprise.my.salesforce.com`)
  - `SALESFORCE_REDIRECT_URI` — OAuth callback URL (only needed for per-user Authorization Code flow; omit for Client Credentials)
  - `MASTER_ENCRYPTION_KEY` — 64-char hex key for multi-tenant credential encryption
  - `SUPER_ADMIN_USERNAME` — Super-admin portal login
  - `SUPER_ADMIN_PASSWORD` — Super-admin portal password
...
└── bot/
    ...
    ├── salesforce-auth.js  # Salesforce OAuth2 (Client Credentials or Authorization Code flow)
    ├── salesforce-api.js   # Salesforce REST API connector (SOQL/SOSL, read + write operations, forecast, competitors)
    ├── salesforce-intents.js # CRM intent handler (fallback when agent unavailable)
    │
    │── # Sales Follow-Up Agent
    ├── sales-agent.js      # Sales module orchestrator (init, intent detection, tool registration)
    ├── sales-analyzer.js   # Pipeline analysis engine (risk scoring, stale/ghost detection, stage inconsistencies)
    ├── sales-tools.js      # 22 agent tools: pipeline analysis, CRM search, write-back, forecast, competitors, alerts
    ├── sales-config.js     # Sales thresholds and constants (stale days, risk weights, amount thresholds)
    ├── sales-templates.js  # Message formatting for Rainbow (markdown templates)
    ├── sales-dashboard.js  # PII visualization dashboard (3-step: raw → anonymized → result) per request
    ├── sales-scheduler.js  # Proactive sales alerts (daily digest, weekly summary, stale deal alerts)
    │
    │── # Email Automation
    ├── email-scheduler.js  # Daily email digest with AI triage, Outlook auto-actions, follow-up tracker
    ├── email-intelligence.js # Priority learning (sender scores) + smart follow-up timing
    │
    │── # Unified Context Management
    ├── context-manager.js  # Cross-path conversation context (entities, history, file refs) in Redis sorted sets
    │
    ...
    ├── user-defaults.js    # Auto-provision new users with default rules, alerts, and digest settings
    │
    │── # Multi-Tenant Platform
    ├── tenant.js           # Tenant CRUD, encrypted credential storage, domain indexing
    ├── tenant-resolver.js  # Fast JID-to-tenant resolution with 3-level LRU cache
    ├── super-admin.js      # Super-admin portal for managing up to 1000 companies
    │
...
- "Jack" → searches → finds "CHEN Jack Lixin" → stores via `update_memory`
- "he" / "him" → resolved from working memory or conversation history
...
For document operations (translate, anonymize, create file), the bot uses **regex-based intent detection** (`detectIntent()`) because these depend on stored file state that the AI can't access. The AI is a pure text engine for these tasks.

#### Intent Detection (`detectIntent()`)
Analyzes the user message for document operations:
...
Full Salesforce pipeline intelligence integrated into the agent via `sales-agent.js`, `sales-tools.js`, `sales-analyzer.js`.

**22 Agent Tools:**
- **Read:** `analyze_pipeline`, `get_deal_risks`, `get_stale_deals`, `get_missing_next_steps`, `get_pipeline_summary`, `get_deal_details`, `get_ghost_deals`, `get_deals_by_owner`, `list_opportunities`, `search_crm`, `get_opportunity_details`, `get_account_details`
- **Write:** `update_opportunity`, `create_task`, `log_activity`, `close_deal` (write ops require user confirmation via Redis pending action)
- **Forecast:** `get_forecast`, `set_quota`
- **Competitors:** `get_competitors`, `add_competitor`, `search_deals_by_competitor`
- **Alerts:** `manage_sales_alerts`

**Salesforce Auth:** Supports Client Credentials flow (`SF_CLIENT_ID` + `SF_CLIENT_SECRET` + `SF_LOGIN_URL`, no redirect needed) and Authorization Code flow (per-user OAuth with redirect URI). Client Credentials is the default when `SALESFORCE_REDIRECT_URI` is not set. Login URL must use org domain (e.g. `https://al-enterprise.my.salesforce.com`).

**Sales Scheduler (`sales-scheduler.js`):** Background timers for proactive alerts:
- Daily pipeline digest (configurable time, timezone-aware, dedup locked)
- Weekly Monday pipeline summary
- Instant stale deal alerts (consolidated top 5, 7-day dedup)
- User prefs in Redis (`sales:schedule:prefs:{userId}`), opt-in set (`sales:schedule:users`)
- Config via uploaded JSON file or chat commands

**PII Dashboard (`sales-dashboard.js`):** Per-request web pages showing data flow:
- `/sales/dashboard/{id}` — overview with 3-step indicators
- `/sales/raw/{id}` — raw Salesforce data
- `/sales/anonymized/{id}` — anonymized version sent to AI
- `/sales/result/{id}` — final AI response (markdown rendered to HTML)
- `/sales/history` — all past reports
- Each request gets a unique session ID; link appended to bot response

### Email Automation

**Email Scheduler (`email-scheduler.js`):** Daily email digest with AI triage:
- Fetches unread emails, classifies with Claude Sonnet into categories: EMT, URGENT, ACTION, FYI, SYSTEM, NOISE
- Custom categories via user-configurable rules stored in Redis (`email:rules:{userId}`)
- Outlook auto-actions per category: flag, categorize, move to folders, mark as read
- Auto-creates Outlook folders (Low Priority, System Emails, custom category folders)
- Follow-up tracker: scans sent emails, alerts on unreplied after 3 days, flags in Outlook
- CRM enrichment: cross-references senders with Salesforce contacts
- Config via `alerts.json` upload or chat commands
- Test endpoint: `/api/email-digest-test?uid=`

**Email Intelligence (`email-intelligence.js`):**
- Priority Learning: tracks read/reply interactions per sender, builds priority scores (high/normal/low), boosts important senders in digests
- Smart Follow-up Timing: tracks when contacts reply (day, hour), suggests best time for follow-ups
- Data in Redis with 30-90 day TTL, learns passively from normal usage

**Agent Email Tools:**
- `get_classified_emails` — on-demand AI classification using custom rules (not just Outlook importance flag)
- `summarize_thread` — AI summary of email conversation with key points, decisions, action items
- `check_followups` — sent emails awaiting reply with days waiting
- `manage_email_rules` — create/remove custom classification rules + Outlook folders (retroactive apply)
- `manage_email_digest` — enable/disable/configure daily digest
- `get_followup_timing` — best time to follow up with a contact based on reply patterns

**Graph.js Extensions:**
- `getUnreadEmailsSince()`, `getSentEmails()` — filtered email queries
- `setCategories()`, `flagWithDueDate()` — Outlook email management
- `resolveOrCreateFolder()` — auto-create Outlook folders
- `moveToFolder()` updated to use `resolveOrCreateFolder`

### Unified Context Management (`context-manager.js`)

Shared conversation context across all AI paths (agent, chat, documents):
- Messages stored in Redis sorted sets (`context:{userId}:messages`) with 24h TTL
- Entities tracked across paths (`context:{userId}:entities`) with 2h TTL
- Session metadata (`context:{userId}:meta`) with 1h TTL
- `getContextForChat()` — injects entities + agent summaries + file refs into Opus system prompt
- `getContextForAgent()` — injects recent conversation summary into Sonnet system prompt (PII-filtered)
- `syncAgentMemory()` — propagates agent working memory entities to unified store
- `generateSummary()` — heuristic 1-2 sentence summary of responses (no AI call)
- Agent now includes last 10 conversation messages (PII-filtered) for follow-up support

### User Defaults & Auto-Provisioning (`user-defaults.js`)

When a user activates via `/admin` invitation:
- Default email digest settings applied (08:00, auto-actions, CRM enrichment)
- Default sales alert settings applied (daily + weekly + stale deal alerts)
- Default email classification rules applied (EMT list)
- Defaults stored in Redis (`defaults:config`), admin-customizable
- Each user can then customize independently via chat

### Multi-Tenant Platform (Phase 1)

**Tenant Model (`tenant.js`):**
- Tenant CRUD with encrypted credential storage (AES-256-GCM)
- Per-tenant Salesforce/M365/Gmail credentials
- Domain-based tenant indexing for JID resolution
- Redis keys: `tenants` (SET), `tenant:{id}`, `tenant:slug:{slug}`, `tenant:domain:{domain}`

**Tenant Resolver (`tenant-resolver.js`):**
- 3-level cache: L1 in-memory LRU (10K entries, 5min TTL) → L2 Redis JID index → L3 Redis domain index
- Extracts domain from Rainbow JID for tenant lookup

**Super-Admin Portal (`super-admin.js`):**
- Web UI at `/super-admin` for platform management
- Tenant CRUD, credential configuration, status management
- Separate JWT auth (`SUPER_ADMIN_USERNAME`/`SUPER_ADMIN_PASSWORD`)

### Message Routing (Updated)

The bot now routes ALL messages through the agent by default:
```javascript
...
```
No more keyword regex for routing. The agent decides which tools to use. Document operations (translate/anonymize/create_file) are the only exception — they use regex detection because they depend on stored file state.

### Markdown Stripping

Rainbow S2S doesn't render HTML. `buildMessage()` calls `stripMarkdown()` to remove `#`, `**`, `*`, backticks from the plain text body so messages display cleanly.

### Debug & Test Endpoints

- `GET /api/sf-debug` — Salesforce auth status (token obtained, instance URL, last error)
- `GET /api/sales-alert-test?uid=` — Manual trigger for sales daily digest
- `GET /api/email-digest-test?uid=` — Manual trigger for email digest
- `GET /api/agent-debug` — Last agent run trace (loops, tools, response)
- `GET /api/agent-test?q=&uid=` — Test agent directly without Rainbow
- `GET /api/agent-status` — Agent availability

### Stop/Cancel Command

User can type `stop`, `cancel`, or `juju stop` to abort a running agent request. The agent checks `cancelledUsers` Set between each tool-calling loop.

### Sales Write Confirmation

Write operations (`update_opportunity`, `close_deal`, `add_competitor`) use a pending-action pattern:
1. Tool stores action in Redis (`sales:pending:{userId}`, 5-min TTL)
2. Agent shows confirmation to user
3. User replies "yes" → `executePendingAction()` executes the Salesforce write
4. User replies "no" → pending action deleted
