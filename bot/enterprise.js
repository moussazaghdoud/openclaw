...
async function getActivationSsoUrl(inviteToken, baseUrl) {
  ...
  const scopes = "openid profile email offline_access Mail.Read Mail.ReadWrite Calendars.Read User.Read";
  ...
  try {
    const tokenResp = await fetch(`https://login.microsoftonline.com/${M365_TENANT_ID}/oauth2/v2.0/token`, {
      ...
      body: new URLSearchParams({
        ...
        scope: "openid profile email offline_access Mail.Read Mail.ReadWrite Calendars.Read User.Read",
      }),
    });
...
async function checkAccess(jid, rainbowEmail) {
  ...
  if (user && user.status === "INACTIVE") {
    console.log(`${LOG} Access denied: user ${user.email} is INACTIVE`);
    return { allowed: false, user };
  }

  // Non-blocking mode: allow everyone, only block INACTIVE users
  // Enterprise tracks users for admin portal but doesn't lock out unknown users
  if (!user) {
    console.log(`${LOG} Unknown user allowed (non-blocking): jid=${jid} email=${rainbowEmail}`);
  }
